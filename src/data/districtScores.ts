export type DistrictScore = {
  ...
  area: "Paris" | "Inner suburb" | "Versailles corridor";
  scores: {
    security: number;
  ...
  rentLevel: "lower" | "medium" | "high" | "very high";
  studentFit: "excellent" | "good" | "mixed" | "weak";
  summary: string;
  caveat: string;
};

export type ScoreKey = keyof DistrictScore["scores"];
export type Weights = Record<ScoreKey, number>;
export type DistrictScoreOverrides = Partial<Record<ScoreKey, number>>;
...
export function mergeScores(
  ...
  scoreOverrides?: DistrictScoreOverrides
): DistrictScore["scores"] {
  return scoreOverrides ? { ...district.scores, ...scoreOverrides } : district.scores;
}
...
export function weightedTotal(
  ...
  const scores = mergeScores(district, scoreOverrides);
...
export const districts: DistrictScore[] = [
  {
    id: "paris-01",
    name: "1st - Louvre",
    code: "75101",
